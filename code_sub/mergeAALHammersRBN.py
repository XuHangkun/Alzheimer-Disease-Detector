...
class MergeAALHammersRBNConfig:
    def __init__(self,
            atlas_roi_path="atlas_roi.csv",
            atlas_folds = [10,10,10],
            out_dim = 3,
            dropout = 0.6
        ):
        self.atlas_roi_path=atlas_roi_path
        self.atlas = ["AAL","Hammers","rBN"]
...
class MergeAALHammersRBNBaseline(nn.Module):
    ...
    def load_models(self,aal_paths=[],hammers_paths=[],rbn_paths=[]):
        # load aal
...
def merge_models():
    ...
    print(aal_paths)
    config = MergeAALHammersRBNConfig(atlas_folds=[1,7,1])
    config.initialize()
    model = MergeAALHammersRBNBaseline(config)
    model.load_models(aal_paths = aal_paths,hammers_paths=hammers_paths,rbn_paths = rbn_paths)
